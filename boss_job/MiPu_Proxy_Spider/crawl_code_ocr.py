...
def parser(return_json=False):
    ...
    net_list = selector.xpath('//tbody/td[@class="tbl-proxy-isp"]/text()')
    ...
    for i in range(len(img_list)):
        ...
        if return_json is not True:
            if type_list[i] == 'HTTP':
                print("IP&PORT ---> %s:%s; 类型为: %s 运营商为:%s" % (ip_list[i], port, type_list[i], net_list[i]))
                ip = 'http://%s:%s' % (ip_list[i], port)
                save_to_db(ip)
            else:
                print("IP&PORT ---> %s:%s; 类型为: %s 运营商为:%s" % (ip_list[i], port, type_list[i], net_list[i]))
                ip = 'https://%s:%s' % (ip_list[i], port)
                save_to_db(ip)
        else:
...
def save_to_db(ip):
    ...
    db = client["fwwb"]
    boss_ip = db["boss_ip"]
    if boss_ip.find_one({'ip': ip}):
        pass
    else:
        boss_ip.insert({'ip': ip})


if __name__ == '__main__':
    parser()
    schedule.every(4).minutes.do(parser)
    while True:
